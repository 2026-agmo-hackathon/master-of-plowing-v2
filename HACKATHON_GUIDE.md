...
- **패널티 (총 3종)**
  ...
  - **경로 추종 오차** — 누적 경로 오차(RMS)가 기록에 불리하게 반영됨
...
| **(3) 웨이포인트 간격** | 인접 두 점의 거리가 **0.05 m 이상 5.0 m 이하** | 해당 웨이포인트 쌍과 실제 거리를 명시해 거부 |
| **(4) 물리 곡률** | 연속 3점의 방위 변화로 곡률을 재고 차량 최소 회전 반경(약 **4.35 m**)과 비교 | 원본 그대로 적재하고 추종 한계 경고 |

> 코드에서는 위 (1)(2)(3)(4)가 각각 Rule 1 · Rule 2 · Rule 3 · Rule 6 으로 구현되어 있습니다
> (`RddfValidator.cpp`). 상수는 `RddfValidator.hpp` 의 `MIN_WAYPOINT_SPACING_M = 0.05`,
> `MAX_WAYPOINT_SPACING_M = 5.0`, `MAX_MACHINE_SPEED_KMH = 7.0`,
> `MIN_MACHINE_SPEED_KMH = 2.05` 입니다.
...
- **레인 간격 / 선회 반경 최적화** — 트랙터 최소 회전 반경 ≈ **4.35 m** (`WHEELBASE_M 2.05 / tan(WHEEL_MAX_RAD 0.44)`). SWATH는 그 이상이어야 부드러운 U-turn 가능
...
주행 중 누적된 **경로 추종 오차(Cross-Track Error, RMS)** 가 최종 기록에 불리하게 반영됩니다.

앱은 매 제어 주기마다 횡방향 편차(LTD)를 텔레메트리로 발행하고 주행이 끝나면
`track_complete`(경과 시간 포함)를 알립니다. **점수 산정과 순위 집계는 앱이 아니라
리더보드 서버가 수행합니다** — 가산 계수를 포함한 정확한 산식은 대회 규칙 공지를
따르세요.

→ 곡선 구간이 듬성하거나 급격한 헤딩 변화가 있으면 Pure Pursuit 추종 오차가 커져 기록이 나빠집니다.

##### 평균 오차를 줄이는 팁

- **직선 구간**은 점을 촘촘히 둘 필요가 없지만, 간격 상한이 **5 m** 이므로 그보다 길게 벌리면 파일이 거부됩니다 (§4.4 규칙 3).
...
- **소요 시간**, **경운 면적률**, **경로 추종 오차**가 함께 점수에 반영됩니다.
- 통과해야 하는 면적률 임계치(예: 95%)는 **없습니다**.
- 다만 면적률이 낮을수록 점수에서 불리하므로, 시간 단축·면적률 확보·평균 오차 최소화를 **함께 최적화**해야 합니다.
- 농지 이탈로 인한 10배속 가속분도 시간에 반영됩니다.
- 가중치와 산식은 리더보드 서버가 적용하며, 대회 규칙 공지를 기준으로 합니다.

### 7.4 평가 방식

- 모든 평가는 **서버 리더보드에 기록된 데이터**를 기준으로 진행됩니다.
- 주행은 에뮬레이터에서 **REC 으로 기록**하고, 완주 후 저장되는 주행 기록을 리더보드에 제출합니다.
  제출용은 암호화본이며 운영진 키로만 열립니다. 함께 받는 평문 CSV 는 본인 분석용입니다.
- 로컬에서 본 시간/면적률은 참고용일 뿐이며, **공식 기록은 리더보드 값**입니다.
- 마감 시각까지 리더보드에 기록된 본인 팀의 최고 기록이 최종 평가 대상입니다.
...
**맵별로 각각 업로드.** 대회는 M1·M2·M3 세 개 공개 맵을 사용하며, 맵마다 해당 RDDF 파일을 따로 업로드합니다(`upload_rddf.sh -f ./1.rddf` 처럼 파일 단위로 지정하며, 별도의 mapId 파라미터는 없습니다). 맵별 점수는 **팀 이름**을 키로 합산됩니다. (§4.5, §7.5)
...
> **Q10.** 평균 오차는 기록에 얼마나 영향이 큰가요?

평균 오차가 커질수록 기록이 나빠집니다. 가산 계수는 리더보드 서버가 적용하므로
정확한 산식은 대회 규칙 공지를 확인하세요.

앱에서 볼 수 있는 값은 대시보드의 **LTD(횡방향 편차)** 와 `/tmp/pp_trace.csv` 의
`cte` 컬럼입니다. 이 값이 곡선 구간에서 한쪽으로 계속 쌓이면 그 구간이 차량의
추종 한계를 넘었다는 뜻입니다.

→ **곡선 구간 웨이포인트 밀도**와 **헤딩 점프 최소화**로 줄일 수 있습니다. (§7.2, §10.6)
